...
void save_model(vector_t* classifiers, vector_t* classes, char* filename, int pack_model, int optimal_iterations, int argn, char **argv)
{
    ...
    if (argn > 0) {
	char buffer[500];
	char *cwd = getcwd(buffer, 500);
	fprintf(output, "# generated in: %s\n", cwd);
	fprintf(output, "# generated by: icsiboost/src/icsiboost");
	for (i=1; i<argn; ++i) {
...
void usage(char* program_name)
{
    ...
    fprintf(stderr,"  --show_correct          with -C: outputs the current accuracy (implies --output_posteriors)\n");
    fprintf(stderr,"  --silent                with -C: outputs final accuracy (implies --output_posteriors)\n");
...
int main(int argc, char** argv)
{
    ...
    int show_correct = 0; // output the current accuracy (implies output_posteriors == 1)
    int silent = 0; // outputs only final accuracy (implies output_posteriors == 1)
    ...
    while((arg=(string_t*)array_shift(args))!=NULL)
	{
	    ...
	    else if(string_eq_cstr(arg,"--show_correct"))
		{
		    output_posteriors = 1;
		    show_correct = 1;
		}
	    else if(string_eq_cstr(arg,"--silent"))
		{
		    output_posteriors = 1;
		    silent = 1;
		}
    ...
    if(classification_mode)
	{
		...
		{
		    line_num++;
		    if (silent && line_num % 100 == 0) {
			fprintf(stderr, "%5d acc:%.2f%%\r", line_num, 100.0*ctok/line_num);
		    }
		    string_chomp(line);
		    ...
		    string_t* gold = array_get(array_of_tokens, array_of_tokens->length-1);
		    //printf("Gold %s\n", token->data);
		    ...
		    for(i=0; i<templates->length; i++)
			{
			    template_t* template=vector_get(templates, i);
			    string_t* token=array_get(array_of_tokens, i);
			    //printf("AAAA %s %d %d\n", token->data, i, array_of_tokens->length);
		    ...
		    if(!dryrun_mode)
			{
			    ...
			    if(classification_output==0)
				{
				    ...
				    for(l=0; l<classes->length; l++)
					{
					    ...
					    if ((score[l] > 0 && example_classes[l] == 0)) erroneous_example = 1;
					    if (score[l]+decision_threshold >= max) {
						max = score[l]+decision_threshold;
						max_class = l;
					    }
					    //fprintf(stdout, "dd %f %f", score[l], max);
					}
				    //fprintf(stdout, " %s\n", erroneous_example ? "ERROR" : "OK" );
				    ...
				    if (!silent) {
					fprintf(stdout,"\n");
					//fprintf(stdout,"AAA %f %d\n", max, max_class);
				    }
				    if (show_correct || silent) {
					//fprintf(stdout,"<%s> <%s>\n",((string_t*)vector_get(classes,max_class))->data, gold->data);
					if (strcmp(((string_t*)vector_get(classes,max_class))->data, gold->data) == 0) {
					    // count correct predictions
					    ctok++;
					}
					if (!silent) {
					    fprintf(stdout,"%s\n",((string_t*)vector_get(classes,max_class))->data);
					    fprintf(stdout, "correct: %d, total: %d (acc: %.2f%%)\n", ctok, line_num, (100.0*ctok)/line_num);
					}
				    }
				}
			    else
    ...
    for(;iteration<maximum_iterations;iteration++)
	{
	    ...
	    if(save_model_at_each_iteration) save_model(classifiers, classes, model_name->data, 0, 0, 0, NULL);
	    fflush(stdout);
	}

    save_model(classifiers, classes, model_name->data, pack_model, optimal_iterations, argc, argv);
